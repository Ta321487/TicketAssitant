...
namespace TA_WPF.ViewModels
{
    ...
    public class AddRouteViewModel : BaseViewModel
    {
        ...
        public AddRouteViewModel(DatabaseService databaseService = null, MainViewModel mainViewModel = null)
        {
            _databaseService = databaseService;
            _mainViewModel = mainViewModel;
            _isFavorite = false; // 默认不收藏
            
            // 初始化命令
            CreateCommand = new RelayCommand(CreateRoute, CanCreateRoute);
        ...
        private async void CreateRoute()
        {
            try
            {
                ...
                if (_databaseService == null)
                {
                    ...
                    LogHelper.LogSystemError("AddRouteViewModel", "数据库服务未初始化，无法保存路线");
                    IsLoading = false;
                    return;
                }
                ...
                // 保存到数据库
                bool success = await _databaseService.AddRouteAsync(route);

                if (success)
                {
                    MessageBoxHelper.ShowInfo("路线创建成功");
                ...
                {
                    MessageBoxHelper.ShowError("路线创建失败，请重试");
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"创建路线异常: {ex.Message}");
                LogHelper.LogError($"创建路线失败: {ex.Message}", ex);
                MessageBoxHelper.ShowError($"创建路线失败: {ex.Message}");
            }
            finally
            {
                IsLoading = false;
