    ...
    public abstract class TicketBaseViewModel : BaseViewModel
    {
        ...
        protected virtual void AddTicket()
        {
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.LogError($"添加 - 打开添加车票窗口时失败: {ex.Message}", ex);
            }
        }
        ...
        protected virtual void EditTicket()
        {
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.LogError($"修改 - 执行车票修改操作时失败: {ex.Message}", ex);
            }
        }
        ...
        protected virtual void EditTicketByDoubleClick(TrainRideInfo ticket)
        {
            ...
            {
                MessageBoxHelper.ShowError($"修改车票时出错: {ex.Message}");
                LogHelper.LogError($"修改 - 双击修改车票(ID:{ticket?.Id})时失败: {ex.Message}", ex);
            }
        }
        ...
        protected virtual async void DeleteTickets()
        {
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.LogError($"删除 - 删除选中的车票(共{selectedTickets.Count}张)时失败: {ex.Message}", ex);
            }
            finally
            {
                // 结束加载状态
        ...
        protected virtual async Task RefreshDataAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.LogError($"刷新 - 刷新车票数据时失败: {ex.Message}", ex);
            }
            finally
            {
                // 结束加载状态
